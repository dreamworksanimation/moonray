...
static bool conesAreEqual(const Cone& c1, const Cone& c2)
{
...
static void printCone(const char* nameOfCone, const Cone& cone)
{
    std::cerr << "Cone " << nameOfCone << " = ( mAxis: (" << cone.mAxis.x << ", " << cone.mAxis.y << ", " 
...
void TestLightTree::testCone()
{
    ...
    // If I combine identical cones, I should get back the same cone
    {
        ...
        const Cone a{ Vec3f(0.f, 0.f, 1.f), /*cos(0rad)*/ 1.f, /*cos(pi/2rad)*/ 0.f, false };
        const Cone b{ a };
        const Cone result = combineCones(a, b);
        printCone("a", a);
        printCone("b", b);
        CPPUNIT_ASSERT(conesAreEqual(a, b));
        std::cerr << "Cone a and Cone b are equal\n";
        printCone("b", b);
        printCone("result", result);
        CPPUNIT_ASSERT(conesAreEqual(b, result));
        std::cerr << "Cone b and the resulting Cone are equal\n";
    }
    // If I combine an empty cone and a non-empty cone, I should get back the non-empty cone
    {
        ...
        const Cone a{ Vec3f(0.f, 0.f, 1.f), /*cos(0rad)*/ 1.f, /*cos(pi/2rad)*/ 0.f, false };
        const Cone result = combineCones(a, emptyCone);
    ...
    // If one cone entirely contains the other cone, we should get back the larger cone, plus any extra emission
    {
        ...
        const Cone largerCone{ Vec3f(0.f, 0.f, 1.f), /*cos(pi/2rad)*/ 0.f, /*cos(0rad)*/ 1.f, false };
        const Cone smallerCone{ Vec3f(0.f, 0.f, 1.f), /*cos(0rad)*/ 1.f, /*cos(pi/2rad)*/ 0.f, false };
        const Cone result = combineCones(largerCone, smallerCone);
        printCone("larger", largerCone);
        printCone("smaller", smallerCone);
        printCone("result", result);
        const Cone expectedCone{ Vec3f(0.f, 0.f, 1.f), /*cos(pi/2rad)*/ 0.f, /*cos(pi/2rad)*/ 0.f, false };
    ...
    // the two axes, and theta_o and theta_e should encompass both cones's scopes
    {
        ...
        // Cone pointing in +x direction, thetaO is 20deg, thetaE is 70deg
        const Cone a{ Vec3f(1.f, 0.f, 0.f), 0.9396926f, 0.34202014f, false };
        // Cone pointing in +y direction, thetaO is 20deg, thetaE is 90deg
        const Cone b{ Vec3f(0.f, 1.f, 0.f), 0.9396926f, 0.f, false };
        // Result: Cone with axis (sqrt2/2, sqrt2/2, 0), thetaO is 65deg (45deg + 20deg), thetaE is 90deg
        const Cone result = combineCones(a, b);
        const Cone expected{ Vec3f(0.7071068f, 0.7071068f, 0.f), 0.42261826f, 0.f, false };
        printCone("a", a);
        printCone("b", b);
        printCone("result", result);
    ...
    // the two axes, and theta_o and theta_e should encompass both cones's scopes
    {
        ...
        // Cone pointing in +x direction, thetaO is 20deg, thetaE is 70deg
        const Cone a{ Vec3f(1.f, 0.f, 0.f), 0.9396926f, 0.34202014f, false };
        // Cone pointing in +y direction, thetaO is 30deg, thetaE is 90deg
        const Cone b{ Vec3f(0.f, 1.f, 0.f), 0.866025403f, 0.f, false };
        // Result: Cone whose axis is at 50deg on the unit circle, thetaO is 70deg ((90deg + 30deg + 20deg) / 2),
        // thetaE is 90deg
        ...
        const Cone result = combineCones(a, b);
        const Cone expected{ Vec3f(0.642787f, 0.766044f, 0.f), 0.342020f, 0.f, false };
        printCone("a", a);
        printCone("b", b);
        printCone("result", result);
        CPPUNIT_ASSERT(conesAreEqual(result, expected));
