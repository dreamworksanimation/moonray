...
RDL2_DSO_ATTR_DECLARE
    ...
    rdl2::AttributeKey<rdl2::String>       attrTexture;
    rdl2::AttributeKey<rdl2::Rgb>          attrGamma;
...
RDL2_DSO_ATTR_DEFINE(rdl2::LightFilter)
    ...
    attrTexture = sceneClass.declareAttribute<rdl2::String>("texture", "", rdl2::FLAGS_FILENAME);
    sceneClass.setMetadata(attrTexture, "comment",
        "File name of the texture applied to the light filter.  "
        "Any file format supported by OpenImageIO can be used.  ");

    attrGamma = sceneClass.declareAttribute<rdl2::Rgb>("gamma", rdl2::Rgb(1.0f));
    sceneClass.setMetadata(attrGamma, "comment",
        "Per-channel gamma used in color-correcting the light filter's texture, if one is present. "
    ...
    sceneClass.setGroup("Properties", attrTexture);
    sceneClass.setGroup("Properties", attrGamma);
