...
public:
    ...
    RenderOutputWriter(const ImageWriteCache::Mode runMode,
        ...
        mRunMode(runMode),
        mCache(cache),
        mCheckpointTileSampleTotals(0),
        mWidth(0),
        mHeight(0),
        mCurrBufferSpec(nullptr),
        mFiles(files),
    ...
    RenderOutputWriter(ImageWriteCache *cache,
        ...
        mErrors(errors),
        mInfos(infos),
        mRunMode(ImageWriteCache::Mode::DEQ),
        mCache(cache),
        mCheckpointTileSampleTotals(0),
        mWidth(0),
        mHeight(0),
        ...
        mFiles(files),
        mFileNameParam(fileNameParam),
        mCallBackCheckpointResumeMetadata(nullptr),
        mCryptomatteBuffer(nullptr),
        mHeatMap(nullptr),
        mWeightBuffer(nullptr),
        mRenderBufferOdd(nullptr),
        mAovBuffers(nullptr),
        mDisplayFilterBuffers(nullptr),
        mSha1Gen(sha1Gen)
    {
...
private:
    ...
    OIIO::ImageOutput::unique_ptr setupOiioImageSetup(
        const std::string &filename,
        ImageWriteCache::ImageWriteCacheTmpFileItemShPtr tmpFileItem,
        bool &result) const;
    ...
    static void addOiioImageSpec(const ImageWriteCacheImageSpec *imgSpec, std::vector<OIIO::ImageSpec> &specs);
    ...
    const ImageWriteCache::Mode mRunMode;
    ImageWriteCache *mCache;
    ...
    ImageWriteCacheBufferSpec *mCurrBufferSpec;

    const std::vector<File> *mFiles;

    const FileNameParam *mFileNameParam;
    const CallBackCheckpointResumeMetadata *mCallBackCheckpointResumeMetadata;

    const pbr::CryptomatteBuffer *mCryptomatteBuffer;
    const scene_rdl2::fb_util::HeatMapBuffer *mHeatMap;
    const scene_rdl2::fb_util::FloatBuffer *mWeightBuffer;
    const scene_rdl2::fb_util::RenderBuffer *mRenderBufferOdd;
    const std::vector<scene_rdl2::fb_util::VariablePixelBuffer> *mAovBuffers;
    const std::vector<scene_rdl2::fb_util::VariablePixelBuffer> *mDisplayFilterBuffers;
        
    scene_rdl2::grid_util::Sha1Gen *mSha1Gen;
};
