...
public:
    ...
    virtual void visitInstance(geom::Instance& i) override {
        ...
        if (mSharedSceneMap.insert(std::make_pair(ref, std::make_unique<std::atomic<bool>>(false))).second) {
        ...
        BVHUserDataList& bvhUserData)
{
    ...
    if (procedural->isReference()) {
        ...
        if (visitedBVHScene.insert(std::make_pair(ref, std::make_unique<std::atomic<bool>>(false))).second) {
