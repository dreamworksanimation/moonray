...
#if !defined(__CUDACC__) && !defined(__METAL__)
...
inline __host__ __device__
float4 lerp(const __gpu_device__ float4& v1, const __gpu_device__ float4& v2, const float t)
{
...
inline __host__ __device__
void swapf(float __gpu_thread__& a, float __gpu_thread__& b)
{
    float tmp = a;
    a = b;
    b = tmp;
...
#ifndef __APPLE__
inline __host__ __device__
float3 operator+(const float3 __gpu_thread__& a, const float3 __gpu_thread__& b)
{
    return {a.x + b.x, a.y + b.y, a.z + b.z};
}

inline __host__ __device__
float3 operator+(const float3 __gpu_thread__& a, const float b)
{
...
inline __host__ __device__
float3 operator*(const float3 __gpu_thread__& v, const float a)
{
    return {v.x * a, v.y * a, v.z * a};
}

inline __host__ __device__
float3 operator*(const float a, const float3 __gpu_thread__& v)
{
    return {v.x * a, v.y * a, v.z * a};
}
...
inline __host__ __device__
void operator/=(float3 __gpu_thread__& v, const float a)
{
    v.x /= a; v.y /= a; v.z /= a;
}

inline __host__ __device__
float dot(const float3 __gpu_thread__& a, const float3 __gpu_thread__& b)
{
    return a.x * b.x + a.y * b.y + a.z * b.z;
}

inline __host__ __device__
float3 cross(const float3 __gpu_thread__& a, const float3 __gpu_thread__& b)
{
...
inline __host__ __device__
float length(const float3 __gpu_thread__& v)
{
    return sqrtf(dot(v, v));
}

inline __host__ __device__
float3 normalize(const float3 __gpu_thread__& v)
{
...
inline __host__ __device__
void makeCoordSystem(const float3 __gpu_thread__& v1,
                     float3 __gpu_thread__* v2,
                     float3 __gpu_thread__* v3)
{
...
inline __host__ __device__
float maxAbsComponent(const float3 __gpu_thread__& v)
{
...
#ifndef __APPLE__
inline __host__ __device__
float4 operator+(const float4 __gpu_thread__& a, const float4 __gpu_thread__& b)
{
    return {a.x + b.x, a.y + b.y, a.z + b.z, a.w + b.w};
}

inline __host__ __device__
float4 operator-(const float4 __gpu_thread__& v)
{
...
inline __host__ __device__
float4 operator*(const float4 __gpu_thread__& v, const float a)
{
    return {v.x * a, v.y * a, v.z * a, v.w * a};
}

inline __host__ __device__
float4 operator*(const float a, const float4 __gpu_thread__& v)
{
    return {v.x * a, v.y * a, v.z * a, v.w * a};
}
#endif
...
inline __host__ __device__
float3 make_float3(const float4 __gpu_thread__& v)
{
    return {v.x, v.y, v.z};
}

inline __host__ __device__
float4 make_float4(const float3 __gpu_thread__& v, const float a)
{
...
inline __host__ __device__
OptixAabb expandAabb(const OptixAabb __gpu_thread__& bb, const float3 __gpu_thread__& p)
{
...
inline __host__ __device__
OptixAabb combineAabbs(const OptixAabb __gpu_thread__& bb0, const OptixAabb __gpu_thread__& bb1)
{
...
struct MetalGPUXform
{
    MetalGPUXform() = default;

    __host__ __device__
    static MetalGPUXform identityXform()
    {
    ...
    __host__ __device__
    static MetalGPUXform rotateToZAxisXform(const float3 __gpu_thread__& dir)
    {
    ...
    __host__ __device__
    float3 transformVector(const float3 __gpu_thread__& v) const
    {
    ...
    __host__ __device__
    float3 transformPoint(const float3 __gpu_thread__& p) const
    {
...
#if defined(__METAL__)
    float3 transformVector(const float3 __gpu_thread__& v) __gpu_device__ const
    {
        return {m[0][0] * v.x + m[0][1] * v.y + m[0][2] * v.z,
                m[1][0] * v.x + m[1][1] * v.y + m[1][2] * v.z,
                m[2][0] * v.x + m[2][1] * v.y + m[2][2] * v.z};
    ...
    __gpu_thread__
    float3 transformPoint(const float3 __gpu_thread__& p) __gpu_device__ const
    {
        return {m[0][0] * p.x + m[0][1] * p.y + m[0][2] * p.z + m[0][3],
                m[1][0] * p.x + m[1][1] * p.y + m[1][2] * p.z + m[1][3],
                m[2][0] * p.x + m[2][1] * p.y + m[2][2] * p.z + m[2][3]};
    ...
    __host__ __device__
    OptixAabb transformAabb(const OptixAabb __gpu_thread__& bb) const
    {
    ...
    {
#if defined(__APPLE__)
        transform[0] = m[0][0];  transform[1] = m[1][0];  transform[2] = m[2][0];
        transform[3] = m[0][1];  transform[4] = m[1][1];  transform[5] = m[2][1];
        transform[6] = m[0][2];  transform[7] = m[1][2];  transform[8] = m[2][2];
        transform[9] = m[0][3]; transform[10] = m[1][3]; transform[11] = m[2][3];
#else
